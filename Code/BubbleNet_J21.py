...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, p, phil, layers):
        ...
        self.u_pred, self.v_pred, self.u_BCs, self.v_BCs = self.net_psi(self.x_tf_psi, self.y_tf_psi, self.t_tf_psi)
        self.p_pred, self.p_BCs_pred, self.poisson = self.net_p(self.x_tf_p, self.y_tf_p, self.t_tf_p)
        self.phil_pred, self.phil_BCs_1_pred, self.phil_BCs_2_pred, self.phil_BCs_3_pred, self.phil_BCs_4_pred, self.phil_BCs_5_pred = self.net_phil(self.x_tf_phil, self.y_tf_phil, self.t_tf_phil)

        self.loss = tf.reduce_sum(tf.square(self.u_tf - self.u_pred)) + \
                    tf.reduce_sum(tf.square(self.v_tf - self.v_pred)) + \
                    tf.reduce_sum(tf.square(self.phil_tf - self.phil_pred)) + tf.reduce_sum(tf.square(self.p_tf - self.p_pred)) + tf.reduce_sum(tf.square(self.poisson)) + \
                    tf.reduce_sum(tf.square(self.u_BCs_pred)) + \
                    tf.reduce_sum(tf.square(self.v_BCs_pred)) + \
                    tf.reduce_sum(tf.square(self.p_BCs_pred)) + \
                    tf.reduce_sum(tf.square(self.phil_BCs_1_pred)) + \
                    tf.reduce_sum(tf.square(self.phil_BCs_2_pred)) + \
                    tf.reduce_sum(tf.square(self.phil_BCs_3_pred)) + \
...
# ==============================================================================================
    ...
    def net_p(self, x, y, t):
        ...
        return p, p_BCs, poisson

    def net_phil(self, x, y, t):
        phil = self.neural_net_phil(tf.concat([x,y,t],1), self.weights_phil, self.biases_phil)
        phil_BCs_1 = phil[1:5]
        phil_BCs_2 = phil[7:10]
        phil_BCs_3 = phil[86:137]
        phil_BCs_4 = phil[160]
        phil_BCs_5 = phil[1932:1940]
        return phil, phil_BCs_1, phil_BCs_2, phil_BCs_3, phil_BCs_4, phil_BCs_5
...
if __name__ == "__main__": 
    ...
    np.savetxt("u_pred_PhysNet_single.txt", np.hstack(u_pred))
    np.savetxt("v_pred_PhysNet_single.txt", np.hstack(v_pred))
    np.savetxt("p_pred_PhysNet_single.txt", np.hstack(p_pred))
    np.savetxt("phil_pred_PhysNet_single.txt", np.hstack(phil_pred))
    
    np.savetxt("u_error_PhysNet_single.txt", np.hstack(error_u_abs))
    np.savetxt("v_error_PhysNet_single.txt", np.hstack(error_v_abs))
    np.savetxt("p_error_PhysNet_single.txt", np.hstack(error_p_abs))
    np.savetxt("phil_error_PhysNet_single.txt", np.hstack(error_phil_abs))
