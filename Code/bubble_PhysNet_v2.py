"""
BubbleNet: Bubble Dynamics Informed Neural Networks <https://hanfengzhai.net/BubbleNet>
        @ Hanfeng Zhai <https://hanfengzhai.net/> | <hz253@cornell.edu>
...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, p, phil, layers):
        ...
        self.u_pred, self.v_pred, self.u_t_pred, self.u_x_pred, self.u_y_pred, self.u_xx_pred, self.u_yy_pred, self.v_t_pred, self.v_x_pred, self.v_y_pred, self.v_xx_pred, self.v_yy_pred, self.u_up_pred, self.u_down_pred, self.v_up_pred, self.v_down_pred = self.net_psi(self.x_tf_psi, self.y_tf_psi, self.t_tf_psi)
        self.p_pred, self.p_x_pred, self.p_y_pred, self.delta_p_pred, self.p_end_pred = self.net_p(self.x_tf_p, self.y_tf_p, self.t_tf_p)
        self.phil_pred, self.phil_t_pred, self.phil_x_pred, self.phil_y_pred, self.phil_xx_pred, self.phil_yy_pred, self.rho_pred, self.mu_pred = self.net_phil(self.x_tf_phil, self.y_tf_phil, self.t_tf_phil)

        epsi = 0.4301026842320369

        self.loss = tf.reduce_sum(tf.square(self.u_tf - self.u_pred)) + \
                    tf.reduce_sum(tf.square(self.v_tf - self.v_pred)) + \
                    tf.reduce_sum(tf.square(self.phil_tf - self.phil_pred)) + \
                    tf.reduce_sum(tf.square(self.p_tf - self.p_pred)) + \
                    tf.reduce_sum(tf.square((1/epsi)*(self.phil_t_pred + self.u_pred*self.phil_x_pred + self.v_pred*self.phil_y_pred) - 1*(self.phil_xx_pred + self.phil_yy_pred)*(1 - ((self.phil_pred*(1-self.phil_pred))/(epsi*tf.sqrt(self.phil_x_pred**2 + self.phil_y_pred**2)))))) + \
                    tf.reduce_sum(tf.square(self.rho_pred*self.u_t_pred + self.rho_pred*(self.u_pred*self.u_x_pred + self.v_pred*self.u_y_pred) + self.p_x_pred - self.mu_pred*(self.u_xx_pred + self.u_yy_pred))) + \
                    tf.reduce_sum(tf.square(self.rho_pred*self.v_t_pred + self.rho_pred*(self.u_pred*self.v_x_pred + self.v_pred*self.v_y_pred) + self.p_y_pred - self.mu_pred*(self.v_xx_pred + self.v_yy_pred))) + \
                    tf.reduce_sum(tf.square(self.u_up_pred)) + \
                    tf.reduce_sum(tf.square(self.u_down_pred)) + \
                    tf.reduce_sum(tf.square(self.v_up_pred)) + \
                    tf.reduce_sum(tf.square(self.delta_p_pred)) + \
                    tf.reduce_sum(tf.square(self.p_end_pred)) # loss function
...
# ==============================================================================================
    def net_psi(self, x, y, t):
        ...
        for time_psi in t_psi
            uu = griddata(X_psi, u.flatten(), (X, Y), method='cubic')
            vv = griddata(X_psi, v.flatten(), (X, Y), method='cubic')

            u_fulltime = [uu, time_u]
            v_fulltime = [vv, time_v]
        ...
        return u, v, u_t, u_x, u_y, u_xx, u_yy, v_t, v_x, v_y, v_xx, v_yy, u_up, u_down, v_up, v_down
    ...
    def net_p(self, x, y, t):
        ...
        for time_p in t_p
            pp = griddata(X_p, p.flatten(), (X, Y), method='cubic')
            p_fulltime = [pp, time_p]

            for i in pp
                pbeg += [i[0], time_p]
                pend += [i[3766], time_p]

        delta_p = (p_beg - p_end) - 10
        p_end = pend - 799.932

        return p, p_x, p_y, delta_p, p_end

    def net_phil(self, x, y, t):
        phil = self.neural_net_phil(tf.concat([x,y,t],1), self.weights_phil, self.biases_phil)

        phil_t = tf.gradients(phil, t)[0]
        phil_x = tf.gradients(phil, x)[0]
        phil_y = tf.gradients(phil, y)[0]
        
        phil_xx = tf.gradients(phil_x, x)[0]
        phil_yy = tf.gradients(phil_y, y)[0]
        ...
        rho = rho_1 + phil*(rho_2 - rho_1)
        mu = mu_1 + phil*(mu_2 - mu_1)

        return phil, phil_t, phil_x, phil_y, phil_xx, phil_yy, rho, mu
# ==============================================================================================
...
if __name__ == "__main__": 
    ...
    np.savetxt("out/u_pred_PhysNet_BCs.txt", np.hstack(u_pred))
    np.savetxt("out/v_pred_PhysNet_BCs.txt", np.hstack(v_pred))
    np.savetxt("out/p_pred_PhysNet_v2.txt", np.hstack(p_pred))
    np.savetxt("out/phil_pred_PhysNet_v2.txt", np.hstack(phil_pred))
    
    np.savetxt("out/u_error_PhysNet_v2.txt", np.hstack(error_u_abs))
    np.savetxt("out/v_error_PhysNet_v2.txt", np.hstack(error_v_abs))
    np.savetxt("out/p_error_PhysNet_v2.txt", np.hstack(error_p_abs))
    np.savetxt("out/phil_error_PhysNet_v2.txt", np.hstack(error_phil_abs))