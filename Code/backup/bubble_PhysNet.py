...
class BubbleNet:
    ...
    def __init__(self, x, y, t, u, v, p, phil, layers):
        ...
        self.u_pred, self.v_pred = self.FlowNet(self.x_tf_psi, self.y_tf_psi, self.t_tf_psi)
        ...
        self.f_pred = self.PhaseNet(self.x_tf_phil, self.y_tf_phil, self.t_tf_phil)

        self.loss = tf.reduce_sum(tf.square(self.u_tf - self.u_pred)) + \
                    tf.reduce_sum(tf.square(self.v_tf - self.v_pred)) + \
                    tf.reduce_sum(tf.square(self.p_tf - self.p_pred)) + \
                    tf.reduce_sum(tf.square(self.phil_tf - self.phil_pred)) + \
                    tf.reduce_sum(tf.square(self.f_pred)) # loss function
...
# ==============================================================================================
    def initialize_NN_psi(self, layers):        
        ...
        for l in range(0,num_layers-1):
            W_psi = self.xavier_init_psi(size=[layers[l], layers[l+1]])
            b_psi = tf.Variable(tf.zeros([1,layers[l+1]], dtype=tf.float32), dtype=tf.float32)
            weights_psi.append(W_psi)
            biases_psi.append(b_psi)
        return weights_psi, biases_psi
        
    def xavier_init_psi(self, size):
        in_dim_psi = size[0]
        out_dim_psi = size[1]        
...
# ==============================================================================================
    def initialize_NN_p(self, layers):        
        ...
        num_layers = len(layers) 
        for l in range(0,num_layers-1):
            W_p = self.xavier_init_p(size=[layers[l], layers[l+1]])
            b_p = tf.Variable(tf.zeros([1,layers[l+1]], dtype=tf.float32), dtype=tf.float32)
            weights_p.append(W_p)
            biases_p.append(b_p)
        return weights_p, biases_p
        
    def xavier_init_p(self, size):
        in_dim_p = size[0]
        out_dim_p = size[1]        
...
# ==============================================================================================
    def initialize_NN_phil(self, layers):        
        ...
        num_layers = len(layers) 
        for l in range(0,num_layers-1):
            W_phil = self.xavier_init_phil(size=[layers[l], layers[l+1]])
            b_phil = tf.Variable(tf.zeros([1,layers[l+1]], dtype=tf.float32), dtype=tf.float32)
            weights_phil.append(W_phil)
            biases_phil.append(b_phil)
        return weights_phil, biases_phil
        
    def xavier_init_phil(self, size):
        in_dim_phil = size[0]
        out_dim_phil = size[1]
    ...
    def neural_net_phil(self, X_phil, weights_phil, biases_phil):
        ...
        for l in range(0,num_layers-2):
            W_phil = weights_phil[l]
            b_phil = biases_phil[l]
...
# ==============================================================================================
    ...
    def net_phil(self, x, y, t):
        ...
        return phil
    #               ***-----------------------------------------------------***
    def PhaseNet(self, x, y, t):

        phil = self.neural_net_phil(tf.concat([x,y,t],1), self.weights_phil, self.biases_phil)
        u, v = self.FlowNet(x, y, t)
        ...
        f = (1/epsi)*(phil_t + u*phil_x + v*phil_y) - \
                1*(phil_xx + phil_yy)*(1 - ((phil*(1-phil))/(epsi*tf.sqrt(phil_x**2 + phil_y**2))))
...
# ==============================================================================================    
    def predict_u(self, x_star, y_star, t_star):
        ...
        tf_dict = {self.x_tf_psi: x_star, self.y_tf_psi: y_star, self.t_tf_psi: t_star}      
    ...
    def predict_p(self, x_star, y_star, t_star):
        ...
        tf_dict = {self.x_tf_p: x_star, self.y_tf_p: y_star, self.t_tf_p: t_star}   
    ...
    def predict_phil(self, x_star, y_star, t_star):
        ...
        tf_dict = {self.x_tf_phil: x_star, self.y_tf_phil: y_star, self.t_tf_phil: t_star}
        phil_star = self.sess.run(self.phil_pred, tf_dict)
...
if __name__ == "__main__": 
    ...
    # Training
    model = BubbleNet(x_train, y_train, t_train, u_train, v_train, p_train, phil_train, layers)
    model.train(10000)
    ...
    np.savetxt("u_pred.txt", np.hstack(u_pred))
    np.savetxt("v_pred.txt", np.hstack(v_pred))
    np.savetxt("p_pred.txt", np.hstack(p_pred))
    np.savetxt("phil_pred.txt", np.hstack(phil_pred))
    
    np.savetxt("u_error.txt", np.hstack(error_u_abs))
    np.savetxt("v_error.txt", np.hstack(error_v_abs))
    np.savetxt("p_error.txt", np.hstack(error_p_abs))
    np.savetxt("phil_error.txt", np.hstack(error_phil_abs))